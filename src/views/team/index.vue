<template>
    <div class="app-container">
      ...
      <el-table :data="tableData.slice( ( pages.currentPage - 1) * pages.pagesize, pages.currentPage * pages.pagesize )"
        ...
        <el-table-column prop='team_id' label='队伍工号' sortable></el-table-column>
        <el-table-column prop='team_name' label='队伍姓名' sortable></el-table-column>
  ...
  <script>
    ...
    export default {
      ...
      methods: {
        ...
        dateStartFormat(row) {
          if(row.team_start){
            var date = new Date(parseInt(row.team_start))
        ...
        dateEndFormat(row) {
          if(row.team_end){
            var date = new Date(parseInt(row.team_end))
            var Y = date.getFullYear() + '-'
            var M = (date.getMonth() + 1) + '-'
            var D = date.getDate()
            return Y + M + D
